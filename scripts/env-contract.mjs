 ...
 *
 * That silence is the whole problem. This repo shipped with five names declared
 * and far more read, so `pnpm dev` started an API with no AUTH_SECRET and a
 * worker whose REDIS_URL had been stripped, leaving it retrying against the
 * default :6379 while the container listens on :6381. Nothing failed loudly at
 * the point of the mistake. The E2E lane never caught it because scripts/e2e.sh
 * sources .env and launches the apps directly, without turbo, so it exercises a
 * different code path than the one the README tells a reader to run.
 *
